...
const fetchStats = async (): Promise<StatsData> => {
  ...
  const safeData = {
    pageviews: data.pageviews || { value: 0, prev: 0 },
    visitors: data.visitors || { value: 0, prev: 0 },
    visits: data.visits || { value: 0, prev: 0 },
    bounces: data.bounces || { value: 0, prev: 0 },
    totaltime: data.totaltime || { value: 0, prev: 0 },
  };

  // Calculate average visit duration in seconds (with safety checks)
  if (safeData.totaltime.value > 0 && safeData.visits.value > 0) {
    const averageVisitDurationSeconds = safeData.totaltime.value / safeData.visits.value;
    ...
    safeData.totaltime.value = averageVisitDurationMinutes;
    safeData.totaltime.prev = (safeData.totaltime.prev || 0) / 60; // Assuming prev is also in seconds
  }
